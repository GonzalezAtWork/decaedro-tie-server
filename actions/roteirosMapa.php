...
$query = "select id_ponto, endereco, gmaps_latitude, gmaps_longitude, pontos.id_roteiro, codigo_abrigo, roteiros.cor from pontos ";
...
$query .= ' order by pontos.id_roteiro ';
...
<form name="form" id="form" method="post">
	...
	<fieldset>
		<legend id="titulo">Mapa de Pontos de Parada por Roteiros</legend>
		...
		function codeAddresses() {
			...
			if(ultimo >= 0){
				codeAddress(enderecos[ultimo].ponto, enderecos[ultimo].codigo, enderecos[ultimo].address, enderecos[ultimo].roteiro, enderecos[ultimo].roteiro_cor, enderecos[ultimo].lat, enderecos[ultimo].long);
			}
		}
		function codeAddress(id_ponto, codigo, address, roteiro, roteiro_cor, lat, lng) {
			...
			if($db_result){
				foreach ($db_result as $row) {
					...
					//	echo "codeAddress('". $row["id_ponto"] ."','". $row["endereco"] ."');";
						echo "{ponto: '". $row["id_ponto"] ."', codigo:'". $row["codigo_abrigo"] ."', address:'". $row["endereco"] ."', roteiro:1 },";
					}else{
					//	echo "codeAddress('". $row["id_ponto"] ."','". $row["endereco"] ."',". $row["gmaps_latitude"] .",". $row["gmaps_longitude"] .");";
						echo "{ponto:'". $row["id_ponto"] ."', codigo:'". $row["codigo_abrigo"] ."', address:'". $row["endereco"] ."',roteiro:'". $row["id_roteiro"] ."',roteiro_cor:'". $row["cor"] ."', lat:". $row["gmaps_latitude"] .",long:". $row["gmaps_longitude"] ."},";
					}
				}
			}
		...
		<div align="left"><span>
<?php
$query = ' select * from roteiros order by id_roteiro';
$db = Database::getInstance();
$db->setQuery($query);
$db->execute();
$db_result = $db->getResultSet();
...
foreach ($db_result as $row) {
	...
	echo "<td width='130px'>";
	if($id_roteiro == $row["id_roteiro"] ){echo '<b>';}
	...
	echo "<img src='";
	echo $icon;
	echo "'> <a href='home.php?action=roteirosMapa&id_roteiro=" . $row["id_roteiro"] . "'>" . $row["nome"] . "</a>";
	if($id_roteiro == $row["id_roteiro"] ){echo '</b>';}
	echo "</td>";
}
echo "</tr></table>";
