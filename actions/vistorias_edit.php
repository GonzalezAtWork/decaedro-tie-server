...
$drop = new Dropdown();
$gravidade = $drop->getHTMLFromQuery('select id_gravidade as code, nome as label from gravidades order by nome', $id_gravidade, true, 'id_gravidade', $disabled . ' style="font-size:10pt; width:250px;"');
$pop_id_tipo = $drop->getHTMLFromQuery('select id_tipo as code, nome as label from pontostipo order by nome', '', true, 'p_id_tipo', $disabled . ' style="font-size:10pt; width:250px;"');
$pop_id_roteiro = $drop->getHTMLFromQuery('select id_roteiro as code, nome as label from roteiros order by nome', '', true, 'p_id_roteiro', $disabled . ' style="font-size:10pt; width:250px;"');
$pop_id_bairro = $drop->getHTMLFromQuery('select id_bairro as code, nome as label from bairros order by nome', '', true, 'p_id_bairro', $disabled . ' style="font-size:10pt; width:250px;"');
?>
  ...
  function mudaCor(posicao, force){
	...
	function tiraTipo(id_tipo) {
		for (t = 0; t < enderecos_tipo.length ;t++) {
			...
			<?php if($agendada != 't' && $executada != 't'){ ?>
			<div align="center">
				<span class="field"><a href="javascript:checa_roteiro()">Inserir Roteiro</a> | SIMAK: <input type="text" name="simak" id="simak" class="medium_small" style="width:120px" value=""> <img onclick="checa_simak()" width="18" height="18" src="http://png.findicons.com/files/icons/2226/matte_basic/32/search.png" title="Procurar Simak"/></span>
			</div>
			<?php } ?>
